...
int
fill_icmp_header(icmpheader_t * header, uint8_t type, uint8_t code,
                 uint32_t rest, int numbytes)
{
        if (header == NULL)
                return -1;
        ...
        header->checksum =
            csum((unsigned short *)header,
                 (sizeof(icmpheader_t) + numbytes) / sizeof(short));

        return 0;
}

int
fill_udp_header(udpheader_t * header, unsigned short src, unsigned short dst,
                int numbytes)
{
        if (header == NULL)
                return -1;
...
int send_ip_packet(ipheader_t * header, char *buf, int numbytes)
{
        ...
        header->ip_sum =
            csum((unsigned short *)packet, sizeofpacket / sizeof(short));
        ...
        if (sendto
            (sd, packet, header->ip_len, 0, (struct sockaddr *)&din,
             sizeof(din)) < 0) {
                perror("sendto()");
